...
var sketchProc = function(processingInstance) {
    with(processingInstance) {
        ...
        var drawEraser = function() {
            ...
            const pg = createGraphics(eraser.w, eraser.h, JAVA2D);
            pg.beginDraw();
            pg.fill(255, 200, 255);
            pg.rect(eraser.x, eraser.y, eraser.w, eraser.h);
            pg.endDraw();
            //imageMode(CENTER);
        ...
        var draw = function() {
            ...
            if (C === bgColor) {
            	fill(255, 200, 255);
            	rect(mouseX, mouseY, drawEraser.eraser.w, drawEraser.eraser.h);
            	fill(0);
            	textSize(pgDimensions.h / 2)
            	textAlign(CENTER, CENTER);
            	text("ERASER", drawEraser.eraser.x, drawEraser.eraser.y);       	
            }            
            fill(currentColor);
            noStroke();
